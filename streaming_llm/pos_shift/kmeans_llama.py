...
def llama_kmeans_attention_forward_v2(
    self,
    hidden_states: torch.Tensor,
    attention_mask: Optional[torch.Tensor] = None,
    position_ids: Optional[torch.LongTensor] = None,
    ...
    # cluster sizes
    number_states = torch.ones(bsz, self.num_key_value_heads, q_len, 1).to(value_states)
    ...
    if past_key_value is not None:
        ...
        key_states = torch.cat([past_key_value[0], key_states], dim=2)
        value_states = torch.cat([past_key_value[1], value_states], dim=2)
        number_states = torch.cat([past_key_value[2], number_states], dim=2)

    # repeat k/v heads if n_kv_heads < n_heads
    key_states = repeat_kv(key_states, self.num_key_value_groups)
    value_states = repeat_kv(value_states, self.num_key_value_groups)
    number_states = repeat_kv(number_states, self.num_key_value_groups)
    ...
    # rescaling mode
    #accum_attn_weights = attn_weights * number_states.transpose(-1, -2)
    ...
    if sq_len >= cache_size: 
        ...
        recent_key = key_states[:,:,-recent_size:,:]
        recent_value = value_states[:,:,-recent_size:,:]
        recent_number = number_states[:,:,-recent_size:,:]
...
def enable_llama_kmeans_attention_v2(model, start=256, recent=1024, cache=2048):
    global start_size, recent_size, cache_size
    start_size = start
    recent_size = recent
    cache_size = cache
    for name, module in reversed(model._modules.items()):
        if len(list(module.children())) > 0:
            enable_llama_kmeans_attention_v2(
                module, start, recent, cache
            )
