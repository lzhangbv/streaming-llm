...
def flash_attn_func(q, k, v, softmax_scale=None, causal=True):
    return FlashAttnFunc.apply(q, k, v, softmax_scale, causal)
...
if __name__ == "__main__":
    ...
    q_ref = qkv_ref[:, :, 0].transpose(1, 2)
    k_ref = qkv_ref[:, :, 1].transpose(1, 2)
    v_ref = qkv_ref[:, :, 2].transpose(1, 2)
    out_ref = torch.nn.functional.scaled_dot_product_attention(q_ref, k_ref, v_ref, is_causal=True)
    ...
    # compare results
    print(f'The average difference of out is {torch.mean(torch.abs(out_ref - out))}')
    print(f'The average difference of dqkv is {torch.mean(torch.abs(qkv_ref.grad - qkv.grad))}')
