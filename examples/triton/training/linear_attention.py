...
if __name__ == "__main__":
    ...
    # torch sdpa
    def flash_attn_fwbw(qkv, dout):
        out = torch.nn.functional.scaled_dot_product_attention(qkv[0], qkv[1], qkv[2], is_causal=True)
        out.backward(dout)
        qkv.grad = None

    # flash attention
    def linear_attn_fwbw(qkv, dout, normalize=True):
        out = chunk_linear_attn(qkv[0], qkv[1], qkv[2], normalize=normalize)
        out.backward(dout)
        qkv.grad = None

    # benchmark
    print('sdpa attn:', triton.testing.do_bench(lambda: flash_attn_fwbw(qkv, dout)))
    print('linear attn:', triton.testing.do_bench(lambda: linear_attn_fwbw(qkv, dout)))
