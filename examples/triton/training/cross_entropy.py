...
if __name__ == "__main__":
    ...
    # fp16 inputs
    x = torch.randn(batch_size * seqlen, vocab_size, device='cuda', dtype=torch.float16)

    # labels
    y = torch.randint(0, vocab_size, (batch_size * seqlen,), device='cuda', dtype=torch.long)

    def pt_func(x_pt, y):
        xentropy_pt = torch.nn.CrossEntropyLoss(reduction="mean")
        out_pt = xentropy_pt(x_pt, y)
        g = torch.ones_like(out_pt)
        out_pt.backward(g)
        return out_pt

    def triton_func(x_triton, y):
        xentropy_triton = CrossEntropyLoss(reduction="mean", inplace_backward=True)
        out_triton = xentropy_triton(x_triton, y)
        g = torch.ones_like(out_triton)
        out_triton.backward(g)
        return out_triton

    # pytorch
    x_pt = x.float().requires_grad_()
    out_pt = pt_func(x_pt, y)

    # triton
    x_triton = x.float().requires_grad_()
    out_triton = triton_func(x_triton, y)

    # compare
    print(f'The maximum difference in the forward pass is {torch.max(torch.abs(out_pt - out_triton))}')
    print(f'The maximum difference in the backward pass is {torch.max(torch.abs(x_pt.grad - x_triton.grad))}')

    # benchmark
    print('torch:', triton.testing.do_bench(lambda: pt_func(x_pt, y)))
    print('triton:', triton.testing.do_bench(lambda: triton_func(x_pt, y)))
