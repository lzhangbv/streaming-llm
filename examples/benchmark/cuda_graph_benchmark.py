"""
Accelerate LLM inference with CUDAGraph and support HF-style batching with left-side padding. 
"""
import torch
...
class CUDAGraphRunner(nn.Module):
    ...
    def capture(self, input_ids, attention_mask, position_ids):
        # warmup
        self.model(
            input_ids=input_ids,
            attention_mask=attention_mask,
...
@torch.no_grad()
def capture_model(model, inputs, token_mask):
    ...
    graph_runner.capture(input_ids, token_mask, position_ids)

    # run the model
    graph_runner(input_ids, token_mask, position_ids)
    return graph_runner

@torch.no_grad()
def generate(model, decode_model, inputs, token_mask, max_new_tokens):
    ...
    for i in range(1, max_new_tokens): 
        outputs = decode_model(next_token, token_mask, position_ids)
        next_token = outputs.logits[:, -1, :].argmax(dim=-1).unsqueeze(1)
...
def main(args):
    ...
    for i in range(start, args.num_samples): 
        ...
        if i < 0: 
            decode_model = capture_model(model, inputs, token_mask)
            print(f"Graph capture time: {time.time() - t0:.2f} seconds")
            continue
    ...
    if args.profile:
        ...
        with prof:
            generate(model, decode_model, inputs, token_mask, args.max_new_tokens)
